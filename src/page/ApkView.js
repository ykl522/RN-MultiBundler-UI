 ...
 * @LastEditTime: 2023-01-11 17:57:10
            ...
            <div style={{ display: 'flex', flexDirection: 'row', marginTop: 15, marginBottom: 15 }}>
                <Button style={{ width: 120 }} loading={loading} onClick={() => {
                    if (fileListRef.current && fileListRef.current.length > 0 && fileListRef.current[0]) {
                        ...
                        let toolPath = path.dirname(__dirname).substring(0, __dirname.lastIndexOf(path.sep)) + path.sep + 'tools' + path.sep + 'apktool_2.7.0.jar'
                        console.log('curDir', toolPath);
                        console.log('path.dirname', path.dirname(filePath));
                        const { exec } = require('child_process');
                        let cmdStr = `java -jar ${toolPath} d -f ${filePath} -o ${path.dirname(filePath)}/apk`
                        console.log('cmdStr', cmdStr);
                        let packageProcess = exec(cmdStr, { cwd: path.dirname(filePath), encoding: 'buffer' }, (error, stdout, stderr) => {
                            const iconv = require('iconv-lite')
                            if (error) {
                                openNotification('bottomRight', '执行出错')
                ...
                <Button style={{ width: 120, marginLeft: 15 }} onClick={() => {
                    if (fileListRef.current && fileListRef.current.length > 0 && fileListRef.current[0]) {
                        remote.shell.openItem(`${path.dirname(fileListRef.current[0].path)}/apk`)
                    }
                }}>跳转解析目录</Button>
                <Button style={{ width: 140, marginLeft: 15 }} loading={loading} onClick={() => {
                    if (fileListRef.current && fileListRef.current.length > 0 && fileListRef.current[0]) {
                        setLoading(true)
                        let t = setTimeout(() => {
                            deleteDir(`${path.dirname(fileListRef.current[0].path)}/apk`)
                            setLoading(false)
                            clearTimeout(t)
